...
DATABASE_URL=mysql+pymysql://${db_admin_login}:${db_admin_password}@${mysql_fqdn}:3306/${db_name}
ENV_EOF

echo "==> Waiting for MySQL Flexible Server on ${mysql_fqdn}..."
i=0
until python3 -c "
import pymysql
pymysql.connect(host='${mysql_fqdn}', user='${db_admin_login}', password='${db_admin_password}', database='${db_name}').close()
" 2>/dev/null; do
    i=$((i+1))
